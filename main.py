...
app.config['SESSION_FILE_DIR'] = os.getenv('SESSION_FILE_DIR', '/opt/render/project/sessions')  # Render disk path
...
def start_session():
    ...
    session['pending_conditional'] = None  # Track pending conditional questions
    return jsonify({
...
def submit_answer():
    ...
    question = None
    if session.get('pending_conditional') and session['pending_conditional']['key'] == key:
        question = session['pending_conditional']
    elif current_index < len(QUESTIONS) and QUESTIONS[current_index]['key'] == key:
    ...
    next_question = None
    session['pending_conditional'] = None  # Reset pending conditional
    if key in CONDITIONAL_QUESTIONS and answer.lower() == CONDITIONAL_QUESTIONS[key]['value'].lower():
        next_question = CONDITIONAL_QUESTIONS[key]['question']
        session['pending_conditional'] = next_question
    else:
        # Set defaults for conditional fields if not applicable
        if key == "has_history" and answer.lower() == "no":
            session['answers']['history'] = "None"
        if key == "has_meds" and answer.lower() == "no":
            session['answers']['medications'] = "None"
        if key == "has_allergy" and answer.lower() == "no":
            session['answers']['allergies'] = "None"
        if key == "fam_hist" and answer.lower() == "no":
            session['answers']['family_history'] = "None"
        if key == "pattern" and answer.lower() == "other":
        ...
        if not session.get('pending_conditional'):
